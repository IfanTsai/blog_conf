...
local metric_requests = cai_conf.metric_requests
local metric_uri      = cai_conf.metric_uri
local metric_latency  = cai_conf.metric_latency
local get_client_ip   = require 'cai'.get_client_ip
local security_shm    = ngx.shared.security_shm

--[[
    If the client sent a malformed HTTP request line,
    which aborted the HTTP protocol parsing.
